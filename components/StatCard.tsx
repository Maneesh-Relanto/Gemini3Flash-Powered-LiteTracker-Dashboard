...
const StatCard: React.FC<StatCardProps> = ({ label, value, trend, icon, description }) => {
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex items-start justify-between group relative">
      <div className="flex-1">
        <div className="flex items-center gap-1.5 mb-1">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{label}</p>
          {description && (
            <div className="relative group/tooltip">
              <svg className="w-3.5 h-3.5 text-slate-300 hover:text-indigo-500 cursor-help transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="p-3 bg-indigo-50 text-indigo-600 rounded-xl shadow-sm shadow-indigo-50 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300">
        {icon}
      </div>
    </div>
  );
